...
kmeans(std::vector<plist_t> &plists,
       ...
       uint32_t CUT_OFF)
{
    auto const &centroids_indexes = seed_selection(plists_indexes, d, plists,
                                                   itfs, TRIALS, CUT_OFF);
    ...
    while (!termination)
    {
        ...
        boost::experimental::parallel::v2::task_region(*conf.executor,
                                                       [&](ds2i::task_region_handle &trh) { // assign plist to best cluster
...
compute_clusters(std::vector<plist_t> &plists, std::vector<double> const &itfs, uint32_t MAX_ITER, uint32_t MAX_REF_SIZE, uint32_t F, uint32_t TRIALS, uint32_t CUT_OFF, DistanceFunction d, SeedSelectionFunction seed_selection, NeedsPartitionPredicate needs_partition)
{
    ...
    while (to_split.size())
    {
        ...
        auto children = std::move(kmeans(plists, parent.plists_indexes(), itfs, d, seed_selection, MAX_ITER, TRIALS, CUT_OFF));
...
int main(int argc, char **argv)
{
    ...
    auto plists = preprocess_collection(bin_coll_fn,
                                        plists_positions,
                                        itfs, U);
    typedef std::function<double(uint32_t, ds2i::cluster const &, std::vector<plist_t> &, std::vector<double> const &)> d_fun_t;
    // distance is defined as 1 - cosine_similarity
    d_fun_t d = [&](uint32_t plist_index, ds2i::cluster const &cluster, std::vector<plist_t> &plists, std::vector<double> const &itfs)
    {
    ...
    auto seed_selection = [&](std::vector<ds2i::cluster::index_t> const &plists_indexes, d_fun_t d, std::vector<plist_t> &plists, std::vector<double> const &itfs, uint32_t TRIALS, uint32_t CUT_OFF)
    {
        ...
        ds2i::cluster seed1_cluster(seed1_index, plists, itfs);
    ...
    auto needs_partition = [&](ds2i::cluster const &c, std::vector<double> const &itfs, uint32_t MAX_REF_SIZE, uint32_t F)
    {
        // adjust F
        if (c.size() <= 3)
            return false;
        ...
        return reference_size > MAX_REF_SIZE || redundance > 0.3; // at most 30% of redundance
    };

    auto tick = ds2i::get_time_usecs();
    auto clusters = compute_clusters(plists, itfs, MAX_ITER, U / DIV, F, TRIALS, CUT_OFF, d, seed_selection, needs_partition); // needs_partition
