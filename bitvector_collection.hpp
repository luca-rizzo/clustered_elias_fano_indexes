...
namespace ds2i
{
    class bitvector_collection
    {
        ...
        // XXX mining purposes only
        size_t ref_space(global_parameters const& params, size_t i) const
        {
            assert(i < size());
            compact_elias_fano::enumerator endpoints(m_endpoints, 0,
                                                     m_bitvectors.size(), m_size,
                                                     params);
            return (i == size() ? m_bitvectors.size() : endpoints.move(i + 1).second)
                   - endpoints.move(i).second;
        }
